...
class attempt_context
{
  public:
     ...
     * @return The result of the operation, which is a @ref couchbase::error and a @ref transaction_get_result.
     */
    virtual std::pair<couchbase::error, transaction_get_result> get(const couchbase::collection& coll, const std::string& id) = 0;
    ...
    std::pair<couchbase::error, transaction_get_result> insert(const couchbase::collection& coll,
                                                               const std::string& id,
     ...
     */
    template<typename Content>
    std::pair<couchbase::error, transaction_get_result> replace(const transaction_get_result& doc, const Content& content)
    {
        if constexpr (std::is_same_v<Content, std::vector<std::byte>>) {
     ...
     */
    virtual couchbase::error remove(const transaction_get_result& doc) = 0;
     ...
     * @return The result of the operation, with is a @ref couchbase::error and a @ref transaction_query_result.
     */
    std::pair<couchbase::error, transaction_query_result> query(const std::string& statement,
     ...
     * @return The result of the operation, with is a @ref couchbase::error and a @ref transaction_query_result.
     */
    std::pair<couchbase::error, transaction_query_result> query(const scope& scope,
  ...
  protected:
    /** @private */
    virtual std::pair<couchbase::error, transaction_get_result> replace_raw(const transaction_get_result& doc,
                                                                            std::vector<std::byte> content) = 0;
    /** @private */
    virtual std::pair<couchbase::error, transaction_get_result> insert_raw(const couchbase::collection& coll,
                                                                           const std::string& id,
                                                                           std::vector<std::byte> content) = 0;
    /** @private */
    virtual std::pair<couchbase::error, transaction_query_result> do_public_query(const std::string& statement,
                                                                                  const transaction_query_options& options,
