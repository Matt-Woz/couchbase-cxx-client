             ...
             const program_arguments& arguments)
{
...

    {
                  ...
                  for (const auto& id : selected_keys) {
                      attempt.get(collection, id, [&attempt, &collection, id, &arguments, &errors](auto ctx, auto res) {
                          if (ctx.ec() == couchbase::errc::transaction_op::document_not_found_exception) {
                              attempt.insert(collection, id, generate_document(arguments.document_body_size), [&errors](auto ctx, auto) {
                                  if (ctx.ec()) {
                              ...
                              attempt.replace(res, generate_document(arguments.document_body_size), [&errors](auto ctx, auto) {
                                  if (ctx.ec()) {
                                      errors[ctx.ec().message()]++;
                                  }
                              });
        ...
        fmt::print("\rScheduled {} transactions with {} GET+[INSERT|REPLACE] operations in {}ms ({}us, {}s)\n",
        ...
        for (auto& promise : results) {
            ...
            if (err.ec()) {
                transactions_errors[fmt::format(
                  "error={}, cause={}", err.ec().message(), err.cause().has_value() ? err.cause().value().message() : "")]++;
            }
        }
        auto exec_end = std::chrono::system_clock::now();

        fmt::print("\rExecuted {} transactions with {} GET+[INSERT|REPLACE] operations in {}ms ({}us, {}s), average latency: {}ms\n",
                   arguments.number_of_transactions,
                   arguments.number_of_keys_per_transaction,
